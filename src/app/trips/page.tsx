    ...
    <div className="min-h-screen bg-[#faf9f6] p-8">
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col items-center justify-center py-8 mb-8">
          ...
          <h2 className="text-3xl md:text-4xl font-extrabold text-gray-900 tracking-tight">Pack to GO</h2>
          <p className="text-gray-600 mt-3 font-medium">세상을 탐험하고, 여정을 공유하세요.</p>
        </div>

        <div className="flex justify-between items-center mb-8">
          <h1 className="text-2xl font-bold text-gray-900 tracking-tight">내 여행 탐색</h1>
        </div>

        {trips.length === 0 ? (
          <div className="text-center py-20 bg-white rounded-3xl border border-gray-200 shadow-sm">
            <Plane className="w-12 h-12 text-gray-400 mx-auto mb-4" />
            <h3 className="text-lg font-medium text-gray-800">아직 계획된 여행이 없습니다</h3>
            <p className="text-gray-500 mt-2">새로운 여행이 게시되기를 기다려주세요!</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {trips.map(trip => (
              <Link href={`/trips/${trip.id}`} key={trip.id}>
                <div className="group relative bg-white border border-gray-200 rounded-3xl p-6 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300 overflow-hidden cursor-pointer">
                  ...
                  <div className="relative z-10">
                    <div className="flex items-start justify-between mb-4">
                      ...
                      <div className="bg-gray-50 px-3 py-1 rounded-full text-xs font-semibold text-gray-700 shadow-sm border border-gray-200">
                        D-{(() => {
                    ...
                    <h2 className="text-xl font-bold text-gray-900 mb-2 group-hover:text-emerald-600 transition-colors line-clamp-1">{trip.title}</h2>
                    <p className="text-gray-500 font-medium mb-6 flex items-center gap-1.5">
                      <MapPin className="w-4 h-4" />
                      {trip.destination}
                    </p>
                    
                    <div className="flex items-center justify-between mt-4">
                      <div className="flex items-center text-sm text-gray-600 bg-gray-50 rounded-xl p-3 border border-gray-100">
                          ...
                          return (
                            <>
                              {displayUsers.map((uid, i) => {
                                ...
                                return (
                                  <div key={i} className="w-8 h-8 rounded-full border-2 border-white bg-emerald-100 flex items-center justify-center text-xs font-bold text-emerald-600 z-10 relative overflow-hidden shadow-sm" title={profile?.displayName || profile?.email || 'Unknown User'}>
                                    {profile?.photoURL ? (
                              ...
                              {extraCount > 0 && (
                                <div className="w-8 h-8 rounded-full border-2 border-white bg-gray-100 flex items-center justify-center text-xs font-bold text-gray-500 z-10 shadow-sm" title={`${extraCount} more participants`}>
                                  +{extraCount}
                                </div>
                              )}
                            </>
